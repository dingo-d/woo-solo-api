 ...
 * @link https://github.com/infinum/eightshift-libs/blob/develop/src/Main/Autowiring.php
 *
 * @package MadeByDenis\WooSoloApi\Core
...
use MadeByDenis\WooSoloApi\Exception\InvalidAutowireDependency;
use MadeByDenis\WooSoloApi\Exception\NonPsr4CompliantClass;
use Exception;
...
/**
 * The class that defines the autowiring process
 */
class Autowiring
{
	/**
	...
	public function buildServiceClasses(array $manuallyDefinedDependencies = [], bool $skipInvalid = false): array
	{
		...
		foreach ($projectReflectionClasses as $projectClass => $reflectionClass) {
			// Skip abstract classes, interfaces & traits, and non-service classes.
			if (
				$reflectionClass->isAbstract() ||
				$reflectionClass->isInterface() ||
				$reflectionClass->isTrait() ||
				!($reflectionClass->implementsInterface(Registrable::class))
			) {
				continue;
			}
	...
	{
		return \array_filter($serviceClasses, static function ($classNamespace) use ($manuallyDefinedDependencies) {
